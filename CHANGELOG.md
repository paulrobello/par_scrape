...
- Markdown pruning via `--prune`: strip navigation bars, footers, link farms, and other boilerplate from each page's Markdown before it is sent to the LLM, typically cutting input tokens 30–60% on docs/product pages with no loss in extracted fields (lowering both cost and latency on every LLM call). Headings, tables, code blocks, and any line containing a digit (prices, specs) are always preserved; only the Markdown sent to the LLM is pruned, so the saved raw file and the `--if-changed` content hash still use the full Markdown. Off by default (ENH-003).
- Incremental re-scrape via `--if-changed`: repeat crawls skip LLM extraction for pages whose content is unchanged since a previous completed run. Each completed page records a SHA-256 of its converted Markdown; a later `--if-changed` run whose page hash matches a prior completed crawl of the same URL copies that run's extracted outputs into the new run's folder and marks the page complete with no LLM call. Changed pages, never-before-seen pages, and pages whose prior output file was deleted fall through to normal extraction. Off by default (ENH-002). The crawl-state schema bumps to v3 for the new `content_hash` column; existing databases are renamed aside to `jobs.sqlite.bak-v2` on first run (crawl history is preserved).
